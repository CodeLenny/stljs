...
(function() {
  ...
  Stl.Image = (function() {
    ...
    Image.prototype.convertFile = function(filePath) {
      var callback, convertSTL2POV, k, locals, options, progressCb, progresscb, ref, start, stderrStr, thenInsertIntoTemplate, thenRenderToImage, thenWriteToTempFile, v;
      options = {
        height: 240,
        width: 320,
        dst: "./result.png"
      ...
      thenRenderToImage = (function(_this) {
        return function(povFileName) {
          var povcmd;
          povcmd = child_proc.spawn('povray', ['-D', "-i/tmp/" + locals.fileName + ".pov", "+FN", "+W" + options.width, "+H" + options.height, "-o" + options.dst, "+Q9", "+AM1", "+A", "+UA"]);
          ...
          return povcmd.on('exit', function(code) {
            ...
            if (code === 0) {
              ...
              if (parsedError != null) {
                ...
                return callback(new Error("Exit code: " + code + " (" + stderrStr + ")"), stderrStr);
              }
            }
          });
        };
